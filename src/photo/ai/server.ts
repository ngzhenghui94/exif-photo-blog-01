import { generateGroqImageQuery } from '@/services/groq';
import {
  AI_IMAGE_QUERIES,
...
export const generateAiImageQueries = async (
  ...
  const titleAndCaption = await generateGroqImageQuery(
          imageBase64,
        ...
        if (textFieldsToGenerate.includes('title')) {
          title = await generateGroqImageQuery(
            imageBase64,
            AI_IMAGE_QUERIES['title'],
          );
        }
        if (textFieldsToGenerate.includes('caption')) {
          caption =  await generateGroqImageQuery(
            imageBase64,
      ...
      if (textFieldsToGenerate.includes('tags')) {
        tags = await generateGroqImageQuery(
          imageBase64,
      ...
      if (textFieldsToGenerate.includes('semantic')) {
        semanticDescription = await generateGroqImageQuery(
          imageBase64,
